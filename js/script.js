...
case 'follow_up':
    ...
    // Manejar varias consultas de seguimiento comunes si no es pregunta de conocimiento
        ...
        nextQuestion = 'follow_up';
    } else if (text.toLowerCase().includes('recetas') || text.toLowerCase().includes('comidas')) {
        botResponse = "Generando recetas y opciones de comidas adaptadas a tus objetivos...";
    ...
    } else if (text.toLowerCase().includes('agua') || text.toLowerCase().includes('hidratación')) {
        if (userData.weight) {
            const waterNeeds = calculateWaterNeeds(userData.weight, userData.activityLevel || 'moderada');
            botResponse = `Para tu peso de ${userData.weight} kg y nivel de actividad ${userData.activityLevel || 'moderada'}, te recomiendo consumir aproximadamente ${waterNeeds} litros de agua al día. Esta cantidad puede variar según el clima, intensidad del ejercicio, alimentación y factores individuales. Recuerda que frutas y verduras también aportan agua a tu hidratación diaria.`;
        } else {
            botResponse = "La hidratación es fundamental para una buena nutrición. Se recomienda beber aproximadamente 2-3 litros de agua al día, o 30-35ml por kg de peso corporal. Si me proporcionas tu peso, puedo darte una recomendación más personalizada.";
        }
    } else if (text.toLowerCase().includes('ejercicio') || text.toLowerCase().includes('entrenar')) {
        if (userData.goal) {
            botResponse = `El ejercicio es un complemento perfecto para tu plan nutricional. Para tu objetivo de ${userData.goal}, te recomendaría ${getExerciseRecommendation(userData.goal)}. ¿Te gustaría un plan de ejercicios más detallado?`;
        } else {
            botResponse = "El ejercicio regular, combinado con una buena nutrición, es clave para la salud general. Idealmente, deberías combinar actividad cardiovascular (150 min/semana de intensidad moderada) con entrenamiento de fuerza (2-3 veces/semana). ¿Tienes algún objetivo específico con el ejercicio?";
        }
    } else if (text.toLowerCase().includes('macros') || text.toLowerCase().includes('macronutrientes')) {
        if (userData.weight && userData.goal) {
            ...
            botResponse = `Para tu objetivo de ${userData.goal} con ${calories} calorías diarias, recomiendo la siguiente distribución de macronutrientes:
...
¿Te gustaría información más detallada sobre fuentes de cada macronutriente o cómo distribuirlos a lo largo del día?`;
        } else {
            botResponse = "Los macronutrientes (proteínas, carbohidratos y grasas) son los nutrientes que proporcionan energía al cuerpo. Si me proporcionas datos como tu peso, altura, edad, género, nivel de actividad y objetivo, puedo recomendarte una distribución personalizada de macronutrientes.";
        }
    } else {
        // Generar pregunta de seguimiento contextual
        botResponse = "Gracias por tu pregunta. ¿Hay algo específico sobre nutrición o alimentación saludable que te gustaría saber? Estoy aquí para ayudarte con información basada en evidencia científica.";
        
        // Añadir pregunta de seguimiento personalizada si tenemos suficientes datos
        if (userData.goal) {
            botResponse += "\n\n¿Te gustaría más información sobre estrategias nutricionales específicas para " + userData.goal + "?";
        }
    }
    break;
    
case 'update_weight':
    const updatedWeight = parseFloat(text.replace(',', '.'));
    if (isNaN(updatedWeight) || updatedWeight <= 0 || updatedWeight > 300) {
        botResponse = "Por favor, ingresa un peso válido en kilogramos (por ejemplo: 70.5).";
    } else {
        userData.weight = updatedWeight;
        saveUserData();
        botResponse = `¡Perfecto! He actualizado tu peso a ${userData.weight} kg. Esto afectará a tus cálculos nutricionales. ¿Te gustaría ver tu plan nutricional actualizado?`;
...
case 'update_height':
    ...
    // Verificar si el usuario ingresó la altura con formato decimal (metros)
    if (text.includes('.') || text.includes(',')) {
        updatedHeight = parseFloat(text.replace(',', '.')) * 100; // Convertir a cm
    } else {
        // Verificar si ingresó directamente en cm
        updatedHeight = parseFloat(text);
    }
    ...
    if (isNaN(updatedHeight) || updatedHeight <= 0) {
        ...
        // Si parece que ingresaron en metros pero olvidaron el punto/coma (175 en lugar de 1.75)
        if (updatedHeight > 3 && updatedHeight <= 300) {
            // Probablemente es en cm, dejar como está
        } else if (updatedHeight < 3) {
            // Está en metros, convertir a cm
            updatedHeight = updatedHeight * 100;
        }
        
        userData.height = updatedHeight;
        saveUserData();
        botResponse = `¡Perfecto! He actualizado tu altura a ${(userData.height/100).toFixed(2)} metros (${userData.height} cm). Esto afectará a tus cálculos nutricionales. ¿Te gustaría ver tu plan nutricional actualizado?`;
        nextQuestion = 'show_updated_plan';
    }
    break;
